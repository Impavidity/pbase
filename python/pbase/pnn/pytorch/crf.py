...
class CRF(nn.Module):
  ...
  def __repr__(self):
    return '{}(num_tags={})'.format(
      self.__class__.__name__, self.num_tags)
  ...
  def forward(self,
    ...
    if mask is not None:
      if tags.size() != mask.size():
        raise ValueError(
          'size of tags and mask must match, got {} and {}'.format(
            tuple(tags.size()), tuple(mask.size)))
